...
class RecitationsRecitersMenuViewModel @Inject constructor(
    ...
    private val _uiState = MutableStateFlow(RecitationsRecitersMenuUiState(
            ...
            versions.forEach { telawa ->
                versionsList.add(
                    Reciter.RecitationVersion(
                        versionId = telawa.narration_id,
                        server = telawa.narration_url,
                        rewaya =
                            if (repo.language == Language.ARABIC) telawa.narration_name_ar
                            else telawa.narration_name_en,
                        count = telawa.rewayah_surah_total,
                        suar = telawa.narration_available_suras
                    )
                )
            }
        ...
        for (i in versions.indices) {
            for (j in rewayat.indices) {
                if (_uiState.value.selectedVersions[j]
                    && versions[i].narration_name_ar.startsWith(rewayat[j])) {
