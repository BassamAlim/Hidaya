...
import bassamalim.hidaya.core.data.dataSources.room.models.City
...
interface CitiesDao {
    ...
    @Query("""
        SELECT * FROM cities
        WHERE country_id = :countryId AND name_ar LIKE '%' || :name || '%' LIMIT 50
        """)
    fun getTopAr(countryId: Int, name: String): List<City>

    @Query("""
        SELECT * FROM cities
        WHERE country_id = :countryId AND name_en LIKE '%' || :name || '%' LIMIT 50
        """)
